...
public abstract class AlarmClockFragment extends DeskClockFragment implements
	...
	private static final String KEY_DEFAULT_SILENT_AFTER = "10";
	public static final String KEY_AUTO_SILENCE = "auto_silence_";

	private static final int REQUEST_CODE_RINGTONE = 1;
	private static final int REQUEST_CODE_PERMISSIONS = 2;
	private static final int REQUEST_CODE_ALARM_SETUP = 3;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		final View v = inflater.inflate(R.layout.alarm_clock_3, container,
				false);
		...
		if (USE_TRANSITION_FRAMEWORK) {
			...
			// / M: Scrap the views in ListView and request layout again, then
			// alarm item will be
			// / attached correctly. This is to avoid the case when some items
			// are not correctly
			// / attached after animation end @{
			mAddRemoveTransition
	...
	private void enterAlarmSetup(Context context, Alarm alarm) {
		...
		startActivityForResult(intent, REQUEST_CODE_ALARM_SETUP);
	}
	...
	private void setDefaultRingtoneUri(Uri uri) {
		...
		if (uri == null) {
			...
			sp.edit()
					.putString(PREF_KEY_DEFAULT_ALARM_RINGTONE_URI,
							uri.toString()).apply();
		}
	}
	...
	private class AlarmItemAdapter extends CursorAdapter {
		...
		public AlarmItemAdapter(Context context, long expandedId,
		...
		public View newView(Context context, Cursor cursor, ViewGroup parent) {
			final View view = mFactory.inflate(R.layout.alarm_time_3, parent,
					false);
			setNewHolder(view);
			return view;
		}
		...
		public synchronized Cursor swapCursor(Cursor cursor) {
			if (USE_TRANSITION_FRAMEWORK
					&& (mAddedAlarm != null || mDeletedAlarm != null)) {
				TransitionManager.beginDelayedTransition(mAlarmsList,
						mAddRemoveTransition);
			}
		...
		private ItemHolder setNewHolder(View view) {
			...
			holder.alarmItem = (RelativeLayout) view
			...
			holder.label = (TextView) view.findViewById(R.id.alarmListTitle);
		...
		public void bindView(final View view, Context context,
			...
			Log.d(TAG, "alarm.hour" + alarm.hour + " alarm.minutes"
					+ alarm.minutes + " alarm.daysOfWeek.getBitSet()"
					+ alarm.daysOfWeek.getBitSet());
			// We must unset the listener first because this maybe a recycled
			// view so changing the
			// state would affect the wrong alarm.

			itemHolder.alarmItem.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					enterAlarmSetup(mContext, alarm);
			...
			if (itemHolder.onoff.isChecked() != alarm.enabled) {
				final ViewGroup onoffParent = (ViewGroup) itemHolder.onoff
						.getParent();
				final int onoffIndex = onoffParent
						.indexOfChild(itemHolder.onoff);
			...
			itemHolder.clock.setFormat(mContext, mContext.getResources()
					.getDimensionPixelSize(R.dimen.alarm_label_size));
	...
	protected void asyncAddAlarm(final Alarm alarm) {
		...
		final AsyncTask<Void, Void, AlarmInstance> updateTask = new AsyncTask<Void, Void, AlarmInstance>() {
			...
			protected AlarmInstance doInBackground(Void... parameters) {
				if (context != null && alarm != null) {
					Events.sendAlarmEvent(R.string.action_create,
							R.string.label_deskclock);
					ContentResolver cr = context.getContentResolver();
	...
	protected void asyncUpdateAlarm(final Alarm alarm, final boolean popToast) {
		...
		final AsyncTask<Void, Void, AlarmInstance> updateTask = new AsyncTask<Void, Void, AlarmInstance>() {
			@Override
			protected AlarmInstance doInBackground(Void... parameters) {
				Events.sendAlarmEvent(R.string.action_update,
						R.string.label_deskclock);
				ContentResolver cr = context.getContentResolver();
	...
	public void setFabAppearance() {
		...
		mFab.setImageResource(R.drawable.fab_alarm_clock);
	...
	public void setSystemAlarmRingtoneToPref() {
		Uri systemDefaultRingtone = RingtoneManager
