...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
      ...
      //Add:set the wall paper color in SharedPreferences
    	if("android.intent.action.wallpaper.LOCKSCREEN_CHANGE".equals(intent.getAction()))
    	{
    		Editor shareData = context.getSharedPreferences("data", 0).edit();
        	int color = intent.getIntExtra("wallpaper_lock_color", 1);
    		shareData.putInt("data", color);
        	shareData.apply();
    	}
    ...
    private void handleIntent(Context context, Intent intent) {
        ...
        try {
        	instance = AlarmInstance.getInstance(context.getContentResolver(),
