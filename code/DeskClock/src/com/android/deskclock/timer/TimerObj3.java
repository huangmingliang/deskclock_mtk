...
public class TimerObj3 implements Parcelable{


    public static final String KEY_NEXT_TIMER_ID = "next_timer_id";

    private static final String TAG = "TimerObj3";
    ...
    public static final Parcelable.Creator<TimerObj3> CREATOR = new Parcelable.Creator<TimerObj3>() {
        @Override
        public TimerObj3 createFromParcel(Parcel p) {
            return new TimerObj3(p);
        }

        @Override
        public TimerObj3[] newArray(int size) {
            return new TimerObj3[size];
        }
    };
    ...
    public TimerObj3(long timerLength, int timerId) {
      init(timerLength, timerId);
    }

    public TimerObj3(long timerLength, Context context) {
        init(timerLength, getNextTimerId(context));
    }

    public TimerObj3(long length, String label, Context context) {
        this(length, context);
    ...
    public static TimerObj3 getTimerFromSharedPrefs(SharedPreferences prefs, int timerId) {
        final TimerObj3 timer = new TimerObj3();
        timer.mTimerId = timerId;
        timer.readFromSharedPref(prefs);
        return timer;
    }

    public static void getTimersFromSharedPrefs(SharedPreferences prefs, List<TimerObj3> timers) {
        ...
        Collections.sort(timers, new Comparator<TimerObj3>() {
            @Override
            public int compare(TimerObj3 timer1, TimerObj3 timer2) {
    ...
    public static void getTimersFromSharedPrefs(SharedPreferences prefs, List<TimerObj3> timers,
        ...
        for (String timerIdString : getTimerIds(prefs)) {
            final int timerId = Integer.parseInt(timerIdString);
            final TimerObj3 timer = getTimerFromSharedPrefs(prefs, timerId);
    ...
    public static void putTimersInSharedPrefs(SharedPreferences prefs, List<TimerObj3> timers) {
        for (TimerObj3 timer : timers) {
            timer.writeToSharedPref(prefs);
        }
    }

    public static void resetTimersInSharedPrefs(SharedPreferences prefs) {
        final List<TimerObj3> timers = new  ArrayList<>();
        getTimersFromSharedPrefs(prefs, timers);
        for (TimerObj3 timer : timers) {
            timer.setState(TimerObj3.STATE_RESTART);
